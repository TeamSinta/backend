...
API_KEY = os.environ.get("DAI_TEST_KEY")
...
class MeetingRoom(APIView):
    ...
    def post(self, request):
        ...
        # When S3 is integrated and usable with our Daily.co storage. This will have to be modifed once we have the
        # actual recording url.
        interview_round.video_uri = f"www.teamsinta.com/{room_name}/epochtime.mp4"
        # ------ Placeholder -----#
        interview_round.save()

        return JsonResponse({"room_data": room_response, "token": token_response["token"]})

    def get(self, request, room_name):
        room_response = get_room_info(room_name)
        if not room_response:
            return JsonResponse({"error": "Invalid room or room has expired."}, status=500)
    ...
    def delete(self, request):
        ...
        batch_response = requests.delete(batch_url, headers=headers, json={"room_names": room_list})
        if not batch_response:
            return JsonResponse({"error:" "Something went wrong with deleting the rooms."}, status=500)
...
class Tokens(APIView):
    ...
    def post(self, request):
        ...
        if not room_name:
            return JsonResponse({"error": "Missing correct 'room_name' in request body."}, status=500)

        room_exists_response = get_room_info(room_name)
        if not room_exists_response:
            return JsonResponse({"error": "Room expired or does not exist."}, status=500)
    ...
    def get(self, request, token):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return JsonResponse({"error": "Token invalid or could not be found."}, status=404)
