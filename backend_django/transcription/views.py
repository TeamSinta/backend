...
class QuestionTranscriptView(APIView):
    def get(self, request: HttpRequest, interview_round_id: int) -> Response:
        ...
        return Response({'questionData': transcripts}, status=status.HTTP_200_OK)
    ...
    def _get_transcripts_for_questions(self, interview_round_id: int) -> List[Dict]:
        ...
        for q in interview_questions:
            ...
            asked_question = tc_by_interviewer.order_by(CosineDistance('embedding', q.embedding)).first()
        ...
        for i in range(1, len(question_start_times)):
            ...
            if tc is not None:
                ...
                if len(relevant_question_chunks) > 0:
                    duration_start = seconds_to_minutes(relevant_question_chunks.first().start_time) 
                    duration_end = seconds_to_minutes(relevant_question_chunks.last().end_time)
            ...
            question_transcript = sorted(question_transcript, key=lambda x: x['timestamp'])
