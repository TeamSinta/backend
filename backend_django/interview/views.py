...
from .models import InterviewRound, Topic, InterviewRoundTopic, InterviewRoundInterviewer, InterviewRoundQuestion
...
def get_all_topics(request):
    if request.method == 'GET':
        ...
        data = [{'id': topic.id, 'topics_text': topic.topics_text, 'company_id': topic.company_id.id} for topic in topics]
    ...
    if request.method == 'POST':
        try:
            ...
            response_data = {
                ...
                'topic_ids': list(interview_round_topics.topic.values_list('id', flat=True)),
...
@csrf_exempt
def update_interview_round_topics(request, interview_round_id, interview_round_topics_id):
    try:
        interview_round_topics = InterviewRoundTopic.objects.get(id=interview_round_topics_id)
...
def get_all_interview_round_interviewers(request, interview_round_id):
    if request.method == 'GET':
        interview_round_interviewers = InterviewRoundInterviewer.objects.filter(interview_round_id=interview_round_id)
        data = [
            {
                'id': interview_round_interviewer.id,
                'interviewer_id': interview_round_interviewer.interviewer_id,
...
def update_interview_round_interviewer(request, interview_round_id, interviewer_id):
    try:
        interview_round_interviewer = InterviewRoundInterviewer.objects.get(id=interviewer_id)
...
def delete_interview_round_interviewer(request, interview_round_interviewer_id):
    try:
        interview_round_interviewer = InterviewRoundInterviewer.objects.get(id=interview_round_interviewer_id)
    except InterviewRoundInterviewer.DoesNotExist:
    ...
    if request.method == 'DELETE':
        ...
        return JsonResponse({'message': 'InterviewerRoundInterviewer deleted successfully'})
    else:
        error_response = {'error': 'Invalid request method'}
        return JsonResponse(error_response, status=405)
...
def create_interview_round_question(request, interview_round_id):
    if request.method == 'POST':
        ...
        interview_round_topics = InterviewRoundTopic.objects.filter(id__in=interview_round_topic_ids)
        ...
        return JsonResponse({'status': 'success', 'message': 'Interview round question created successfully'})
    else:
...
def get_all_interview_round_questions(request, interview_round_id):
    if request.method == 'GET':
        ...
        interview_round_questions = InterviewRoundQuestion.objects.filter(interview_round_id=interview_round_id)
        ...
        for interview_round_question in interview_round_questions:
            data.append({
                ...
                'interview_round_topic_ids': [topic.id for topic in interview_round_question.interview_round_topics.all()],
...
def update_interview_round_question(request, interview_round_question_id):
    try:
        interview_round_question = InterviewRoundQuestion.objects.get(id=interview_round_question_id)
    ...
    if request.method == 'PUT':
        try:
            ...
            if interview_round_topics_id:
                ...
                interview_round_question.interview_round_topics.add(interview_round_topics)

            # Construct the response data
            response_data = {
                'id': interview_round_question.id,
                'interview_round_id': interview_round_question.interview_round_id,
                'interview_round_topics_id': list(interview_round_question.interview_round_topics.values_list('id', flat=True)),
                'question_id': interview_round_question.question_id,
                'created_at': interview_round_question.created_at,
                'updated_at': interview_round_question.updated_at
            }
...
@csrf_exempt
def delete_interview_round_question(request, interview_round_id, interview_round_topics_id, interview_round_question_id):
    try:
        interview_round_question = InterviewRoundQuestion.objects.get(id=interview_round_question_id)
